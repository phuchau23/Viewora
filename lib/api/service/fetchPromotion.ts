...
export const PromotionService = {
  ...
  getPromotionById: async (id: string): Promise<PromotionResponse> => {
    try {
      const response = await apiService.get<PromotionResponse>(
        `/promotions/${id}`
      );
      return response.data;
    } catch (error: any) {
  ...
  getDiscountTypes: async (
    pageIndex: number = 1,
    pageSize: number = 10
  ): Promise<DiscountTypeListResponse> => {
    try {
  ...
  createPromotion: async (formData: FormData): Promise<PromotionResponse> => {
    const response = await apiService.post<PromotionResponse>(
      "/promotions",
      formData
    );
    return response.data;
  ...
  updatePromotion: async (
    ...
    try {
      ...
      const response = await apiService.put<PromotionResponse>(
        `/promotions/${id}`,
        formData
      );
  ...
  getPromotionsAvailable: async (
    userId: string
  ): Promise<{ data: Promotion[] }> => {
    const response = await apiService.get<{ data: Promotion[] }>(
      `/promotions/available/${userId}`
    );
    return response.data;
  },
};
